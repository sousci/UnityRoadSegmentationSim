...
- 3x3 道路グリッド上で自動走行しながら教師データを連続保存
...
| L | 連続キャプチャ ON / OFF |
| B | 決め打ちルートの自動走行 + 教師データキャプチャ ON / OFF |
| H | 操作ガイドの表示 / 非表示 |
| R | 車両位置リセット |
| Esc | ポーズ / 再開 |
...
`B` キーを押すと、固定 waypoint ルートを自動走行しながら RGB / mask / metadata を保存します。自動走行中は速度、ステアリング、車載カメラ角度に小さなランダム揺らぎを入れるため、同じ地図でも教師データの見え方にバリエーションが出ます。
